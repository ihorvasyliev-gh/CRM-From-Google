...
>(({ className, sideOffset = 4, ...props }, ref) => (
  // Portaled to <body>: cards use `content-visibility: auto` (paint containment), which would
  // otherwise clip the tooltip or let neighbouring cards paint over it.
  <TooltipPrimitive.Portal>
  <TooltipPrimitive.Content
    ref={ref}
    sideOffset={sideOffset}
    collisionPadding={8}
    className={`z-[10050] max-w-xs whitespace-pre-line overflow-hidden rounded-md border border-border-subtle bg-surface-elevated px-3 py-1.5 text-xs font-medium text-primary shadow-md animate-in fade-in-0 zoom-in-95 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 ${className || ''}`}
    {...props}
  />
  </TooltipPrimitive.Portal>
));
