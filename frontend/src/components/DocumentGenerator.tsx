            ...
            <div className="bg-white rounded-2xl shadow-card border border-surface-200/60 overflow-hidden">
                ...
                <div className="p-5 border-b border-surface-100">
                    <div className="flex items-start gap-3">
                        ...
                        <div>
                            <h3 className="text-lg font-bold text-surface-900">Word Document Template</h3>
                ...
                {/* Current Template */}
                <div className="px-5 py-3 bg-surface-50/50 border-b border-surface-100">
                    {template ? (
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                                <FileArchive size={16} className="text-violet-500" />
                                <span className="text-sm font-semibold text-surface-700">Current template:</span>
                                <span className="text-sm text-surface-500">{template.name}</span>
                            </div>
                            <button
                ...
                <div className="px-5 py-4">
                    <button
                        ...
                        className="flex items-center gap-2 text-sm font-bold text-surface-700 hover:text-surface-900 transition-colors"
                    >
                        <Info size={14} />
                        Available Variables
                    ...
                    {showPlaceholders && (
                        <div className="mt-3 p-4 bg-surface-50 rounded-xl border border-surface-200/60">
                                        ...
                                        <div className="space-y-1">
                                            {cat.items.map(item => (
                                                <div key={item.key} className="flex items-center gap-2 text-[13px]">
                                                    ...
                                                    <span className="text-surface-600">{item.desc}</span>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
            ...
            <div className="bg-white rounded-2xl shadow-card border border-surface-200/60 overflow-hidden">
                <div className="p-5 border-b border-surface-100">
                    <div className="flex items-start gap-3">
                        ...
                        <div>
                            <h3 className="text-lg font-bold text-surface-900">Generate Documents</h3>
                            <p className="text-sm text-surface-400 mt-0.5">
                ...
                <div className="p-5 space-y-5">
                    ...
                    <div>
                        ...
                        <div className="relative">
                            <button
                                ...
                                className="w-full flex items-center justify-between px-4 py-3 bg-surface-50 border border-surface-200 rounded-xl text-sm transition-all hover:border-brand-300 focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-400"
                            >
                                <span className={selectedCourse ? 'text-surface-800 font-medium' : 'text-surface-400'}>
                                    {selectedCourse?.name || 'Choose a course...'}
                                </span>
                                <ChevronDown size={16} className={`text-surface-400 transition-transform ${courseDropdownOpen ? 'rotate-180' : ''}`} />
                            </button>

                            {courseDropdownOpen && (
                                <div className="absolute z-30 top-full left-0 right-0 mt-1 bg-white rounded-xl shadow-lg border border-surface-200 py-1 max-h-64 overflow-y-auto animate-scaleIn origin-top">
                                        ...
                                        coursesWithConfirmed.map(c => {
                                            ...
                                            return (
                                                <button
                                                    ...
                                                    className={`w-full flex items-center justify-between px-4 py-2.5 text-left text-sm hover:bg-surface-50 transition-all ${selectedCourseId === c.id ? 'bg-brand-50 text-brand-700 font-medium' : 'text-surface-700'
                                                        }`}
                                                >
                                                    <span>{c.name}</span>
                                                    <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${selectedCourseId === c.id ? 'bg-brand-100 text-brand-600' : 'bg-surface-100 text-surface-500'
                                                        }`}>
                                                        {count} confirmed
                                                    </span>
                                                </button>
                    ...
                    {selectedCourseId && confirmedForCourse.length > 0 && (
                        <div>
                            ...
                            <div className="bg-surface-50 rounded-xl border border-surface-200/60 overflow-hidden">
                                <div className="overflow-x-auto max-h-80 overflow-y-auto">
                                    <table className="w-full text-sm">
                                        <thead className="sticky top-0 z-10">
                                            <tr className="bg-surface-100/80 backdrop-blur-sm">
                                                <th className="text-left px-4 py-2.5 text-xs font-bold text-surface-500 uppercase tracking-wider">#</th>
                                                <th className="text-left px-4 py-2.5 text-xs font-bold text-surface-500 uppercase tracking-wider">Name</th>
                                                <th className="text-left px-4 py-2.5 text-xs font-bold text-surface-500 uppercase tracking-wider">Email</th>
                                                <th className="text-left px-4 py-2.5 text-xs font-bold text-surface-500 uppercase tracking-wider">Phone</th>
                                                <th className="text-left px-4 py-2.5 text-xs font-bold text-surface-500 uppercase tracking-wider">Variant</th>
                                                <th className="text-left px-4 py-2.5 text-xs font-bold text-surface-500 uppercase tracking-wider">Confirmed</th>
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y divide-surface-100">
                                            {confirmedForCourse.map((enrollment, idx) => (
                                                <tr key={enrollment.id} className="hover:bg-white/80 transition-colors">
                                                    <td className="px-4 py-2.5 text-surface-400 text-xs font-mono">{idx + 1}</td>
                                                    <td className="px-4 py-2.5 font-medium text-surface-800">
                                                        {enrollment.students?.first_name} {enrollment.students?.last_name}
                                                    </td>
                                                    <td className="px-4 py-2.5 text-surface-500">{enrollment.students?.email || '—'}</td>
                                                    <td className="px-4 py-2.5 text-surface-500">{enrollment.students?.phone || '—'}</td>
