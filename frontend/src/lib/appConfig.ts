...
function getEmailWrapper(content: string, type: 'invite' | 'status') {
    ...
    const heroClass = isInvite ? 'hero-gradient' : 'hero-gradient-status';
    ...
    // Add VML namespace to <html> for Outlook button support!
    return `<!DOCTYPE html>
...
<head>
  ...
  <style>
    ...
    .hero-gradient-status { background: linear-gradient(135deg, #8b5cf6 0%, #6d28d9 100%); }
          ...
          <table role="presentation" class="shadow" style="width:94%;max-width:600px;border:none;border-spacing:0;text-align:left;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Arial,sans-serif;font-size:16px;line-height:22px;color:#363636;background-color:#ffffff;border-radius:16px;overflow:hidden;">
            ...
            <tr>
              <td style="padding:24px 32px;text-align:center;background-color:#ffffff;border-bottom:1px solid #f0f0f0;">
                <img src="${origin}/logos-banner.png" alt="Cork City Partnership — Government of Ireland, EU Co-Funded, SICAP" width="500" style="width:100%;max-width:500px;height:auto;border:none;text-decoration:none;color:#ffffff;">
              </td>
            </tr>
            <!-- Hero -->
            <tr>
              <td class="${heroClass}" style="padding:48px 32px;text-align:center;background-color:${heroBg};">
